...
/*
   string st_code[59] = {"al","ak","as","az","ar","ca","co","ct","de","dc","fl","ga","gu","hi","id","il","in","ia","ks","ky","la","me","md","mh","ma","mi","fm","mn","ms","mo","mt","ne","nv","nh","nj","nm","ny","nc","nd","mp","oh","ok","or","pw","pa","pr","re","sc","sd","tn","tx","ut","vt","va","vi","wa","wv","wi","wy"};
        for(int i = 0; i < 59; i++){
        plateResult.region = st_code[i];
        country_recognizers.ocr->performOCR(&pipeline_data);
            country_recognizers.ocr->postProcessor.analyze(plateResult.region, topN);

             const vector<PPResult> ppResults1 = country_recognizers.ocr->postProcessor.getResults();

             for (unsigned int pp = 0; pp < ppResults1.size(); pp++)
                     {
                        if(ppResults1[pp].matchesTemplate){
                        std::cerr << "\n States matching templates - " <<st_code[i];
                        break;
                        }
                     }
        }
