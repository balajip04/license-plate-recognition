...
namespace alpr
{
  ...
  {
      std::cerr << "config_file alpr:" << config_file <<  endl;
...
std::cerr << "configFile: " << configFile << std::endl;
    ...
    if (fileExists(CONFIG_FILE_TEMPLATE_LOCATION))
    {
      ...
      defaultIni = &defaultIniObj;
            std::cerr << "defaultIni: " << defaultIni << std::endl;
    }

    std::cerr << "DEFAULT_RUNTIME_DATA_DIR: " << DEFAULT_RUNTIME_DATA_DIR << "-ini-" << ini << "-default-" << defaultIni << std::endl;
