...
int main( int argc, const char** argv )
{
  ...
  try
  {
    ...
    std::cerr << "detectRegion  - " << detectRegionSwitch.getValue();
    std::cerr << "templatePatternArg  - " << templatePatternArg.getValue();
    detectRegion=true;
...
bool detectandshow( Alpr* alpr, cv::Mat frame, std::string region, bool writeJson)
{
  ...
  else
  {
    ...
    {
      std::cout << "\n plate" << i << ": " << results.plates[i].topNPlates.size() << " results";
      if (measureProcessingTime)
